...
const chromeStub = {
  ...
  tabs: {
    ...
    async query(q = {}) {
      if('groupId' in q) throw new Error('Arc: tabs.query does not support groupId');
      let out = [...tabs.values()];
...
// ── v0.2's existing no-op stub, verbatim from arc-sidepanel-shim.js ──
chromeStub.tabGroups = {
  Color: { GREY:"grey", BLUE:"blue", RED:"red", YELLOW:"yellow", GREEN:"green", PINK:"pink", PURPLE:"purple", CYAN:"cyan", ORANGE:"orange" },
  TAB_GROUP_ID_NONE: -1,
  get: async () => ({}), update: async () => ({}), query: async () => ([]), move: async () => ({})
};
chromeStub.tabs.group = async () => -1;
chromeStub.tabs.ungroup = async () => {};
...
console.log(`\n${results.length - failed}/${results.length} passed (real 1.0.90 code on v0.2's no-op stub)`);

// This is a control, not a regression suite: the failures above ARE the result.
// It documents why arc-tabgroups.js has to exist. If the stub ever starts
// passing, either upstream stopped using tab groups or the control is broken.
const EXPECTED_MAX_PASSES = 3;
if (results.length - failed > EXPECTED_MAX_PASSES) {
  console.error(`\n✗ control unexpectedly passed ${results.length - failed} checks (max ${EXPECTED_MAX_PASSES}) — re-evaluate whether the shim is still needed`);
  process.exit(1);
}
console.log('✓ control behaved as expected: the no-op stub cannot carry 1.0.90');
process.exit(0);
